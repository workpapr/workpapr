...
export const reportCommand = new Command("report")
  ...
  .option("--ai", "Include AI analysis summary and workpaper references")
  .action(
    async (options: { dir: string; json?: boolean; format?: string; ai?: boolean }) => {
      ...
      if (options.format === "md") {
        ...
        let report = formatMarkdownReport(findings);

        // Append AI section and workpaper references
        if (options.ai) {
          const aiFindings = findings.filter((f) => f.aiDetected);
          if (aiFindings.length > 0) {
            report += "\n## AI-Detected Findings\n\n";
            report += `${aiFindings.length} finding(s) detected by AI analysis.\n\n`;
            for (const f of aiFindings) {
              report += `- **${f.type}** in \`${f.file}:${f.line}\` (${f.severity}) — ${f.context}\n`;
              if (f.aiConfidence) {
                report += `  - Confidence: ${f.aiConfidence.level}\n`;
              }
            }
            report += "\n";
          }
          ...
          if (fs.existsSync(wpDir)) {
            ...
            if (wpFiles.length > 0) {
              report += "## Workpapers\n\n";
              report += "Detailed per-system audit workpapers are available:\n\n";
              for (const wp of wpFiles) {
                report += `- [${wp}](.workpapr/workpapers/${wp})\n`;
              }
              report += "\n";
            }
          }
